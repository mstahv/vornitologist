...
public class ObservationDetailPopover extends Popover {
    ...
    public ObservationDetailPopover(final Observation obs) {
        observation = obs;
        tr = Translations.get(VornitologistUI.getApp().getLocale());
        ...
        Button detailsLink = new Button("...", new ClickListener() {
            public void buttonClick(ClickEvent event) {
                ...
                UI.getCurrent().getNavigator()
                        .navigateTo(obs.getSpecies().getName());
                removeFromParent();
            }
        });
        cssLayout2.setCaption(tr.getString("species") + ": ");
        detailsLink.setCaption(tr.getString(obs.getSpecies().getName()));
        detailsLink.setStyleName(BaseTheme.BUTTON_LINK);
        navigationView.setCaption(tr.getString(obs.getSpecies().getName()));
        ...
        Button placeLink = new Button("...", new ClickListener() {
            public void buttonClick(ClickEvent event) {
                ...
                parent.getMapView().showObservation(obs);
                removeFromParent();
            }
        });
        cssLayout2.setCaption(tr.getString("Observation location") + ": ");
        placeLink.setCaption(obs.getLocation().getName() + " ("
                + df.format(obs.getLocation().getLongitude()) + ", "
                + df.format(obs.getLocation().getLatitude()) + ")");
        ...
        DateFormat dateTimeInstance = SimpleDateFormat.getDateTimeInstance(
                SimpleDateFormat.SHORT, SimpleDateFormat.SHORT, VornitologistUI
                        .getApp().getLocale());
        label.setValue(dateTimeInstance.format(obs.getObservationTime()));
        detailsLayout.addComponent(label);

        label = new Label();
        label.setCaption(tr.getString("observer") + ": ");
        label.setValue(obs.getObserver());
        // label.setDebugId("observerlabel");

        detailsLayout.addComponent(label);

        if (obs.getImage() != null) {
            final File file = new File(obs.getImage());
            if (file.exists()) {
