...
public class ObservationDB {
    ...
    private static final int RANDOM_OBSERVATION_TIME = 365 * 24 * 60 * 60
            * 1000;
    public static final ObservationPoint DEFAULT_TOPLEFT;
    public static final ObservationPoint DEFAULT_BOTTOMRIGHT;
    private static final String[] OBSERVERS = "Eräjorma,Joonas,Saska,Pekka,Marko"
            .split(",");
    static {
        DEFAULT_BOTTOMRIGHT = new ObservationPoint();
        DEFAULT_BOTTOMRIGHT.setLatitude(60);
        DEFAULT_BOTTOMRIGHT.setLongitude(31.2);
        DEFAULT_TOPLEFT = new ObservationPoint();
    ...
    public static List<Observation> getObservations(
            final Species requestedSpecies, ObservationPoint topLeft,
            ObservationPoint bottomRight, int maxResults, int minResults) {
    ...
    private static List<Observation> filterObservations(
            Species requestedSpecies, ObservationPoint topLeft,
            ObservationPoint bottomRight, int maxResults) {
    ...
    private static ObservationPoint getRndPoint(ObservationPoint topLeft,
            ObservationPoint bottomRight) {
        if (topLeft == null) {
    ...
    public static Container getSpeciesContainer(Locale locale) {
        ...
        indexedContainer.addContainerProperty("latin", String.class, "");
        indexedContainer.addContainerProperty("group", String.class, "");
        ...
        for (int i = 0; i < getAllSpecies().size(); i++) {
            ...
            ClassificationGroup group = idByIndex.getParent();
            indexedContainer.getItem(idByIndex).getItemProperty("name")
                    .setValue(tr.getString(idByIndex.getName()));
            indexedContainer.getItem(idByIndex).getItemProperty("latin")
                    .setValue(idByIndex.getName());
            indexedContainer.getItem(idByIndex).getItemProperty("group")
            .setValue(tr.getString(group.getName()));
        }
        return indexedContainer;
    }

    public static void persist(final Observation observation) {
        synchronized (observations) {
            observations.add(observation);
        }
        synchronized (activeContainers) {
            for (final BeanItemContainer<Observation> container : activeContainers
                    .keySet()) {
                UI app = activeContainers.get(container);
                if (app != null && app.isAttached()) {
